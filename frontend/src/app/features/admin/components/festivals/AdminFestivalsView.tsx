...
import { Calendar, Plus, Edit, Trash2, Check, Star, Upload } from 'lucide-react';
...
export const AdminFestivalsView: React.FC = () => {
  ...
  const { currentPage, totalPages, pageItems, goToPage, goToFirstPage } = usePagination(filteredFestivals, 10);

  // Reset to first page when search/month filters change
  useEffect(() => {
    goToFirstPage();
  }, [searchQuery, selectedMonth, goToFirstPage]);
    ...
    >
      <div className="space-y-6">
        ...
        <Modal
          ...
          title={editingFestival ? 'Edit Festival Event' : 'Add New Festival Event'}
        ...
        >
          <form onSubmit={handleSave} className="space-y-4 font-serif text-xs">
            ...
            <TextArea
              label="Spiritual Blessing & Significance"
              labelClassName="block text-stone-800 font-bold mb-1"
              variant="cream"
              rows={2}
        ...
        <ConfirmDialog
          ...
          onConfirm={() => {
            if (deleteConfirm) deleteFestival(deleteConfirm.id);
