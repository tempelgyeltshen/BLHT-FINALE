...
import { 
  Search, Menu, X, ChevronRight, ChevronDown, MapPin, Calendar, 
  BookOpen, Compass, Building2, Sparkles, Mountain, Video, Image as ImageIcon, Car, Brush, MessageCircle
} from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (mobileMenuOpen || isSearchExpanded || isPackagesDropdownOpen || isMediaDropdownOpen || isHQCompaniesDropdownOpen) {
        setIsVisible(true);
        return;
      }
  ...
  }, [mobileMenuOpen, isSearchExpanded, isPackagesDropdownOpen, isMediaDropdownOpen, isHQCompaniesDropdownOpen]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (hqCompaniesContainerRef.current && !hqCompaniesContainerRef.current.contains(event.target as Node)) {
        setIsHQCompaniesDropdownOpen(false);
      }
    };
  ...
  const isHQCompaniesActive = ['car-rental', 'thangka-painting'].includes(currentRoute);
    ...
    <header className={`sticky top-0 z-50 bg-[#fcf8f2] backdrop-blur-md border-b border-[#e2d1be] transition-transform duration-300 relative ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <nav className="hidden lg:flex items-center justify-center gap-6 lg:gap-8 py-3 text-xs font-serif tracking-[0.1em] text-[#5c3820]">
          ...
          {/* HQ COMPANIES DROPDOWN */}
          <div 
            className="relative py-1"
            ref={hqCompaniesContainerRef}
            onMouseEnter={() => setIsHQCompaniesDropdownOpen(true)}
            onMouseLeave={() => setIsHQCompaniesDropdownOpen(false)}
          >
            <button
              onClick={() => setIsHQCompaniesDropdownOpen(!isHQCompaniesDropdownOpen)}
              className={`flex items-center gap-1 font-serif text-xs tracking-[0.1em] cursor-pointer transition-colors py-1 ${
                isHQCompaniesActive ? 'text-[#d96b27] font-bold border-b-2 border-[#d96b27]' : 'text-[#5c3820] hover:text-[#d96b27]'
              }`}
            >
              <span>HQ COMPANIES</span>
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isHQCompaniesDropdownOpen ? 'rotate-180 text-[#d96b27]' : 'text-[#5c3820]'}`} />
            </button>

            {isHQCompaniesDropdownOpen && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-80 bg-[#fcf8f2] border-2 border-[#d96b27] rounded-2xl shadow-2xl p-2.5 z-50 animate-in fade-in zoom-in-95 duration-200 space-y-1 divide-y divide-[#efe2d3]/80">
                <div 
                  onClick={() => { navigate('car-rental'); setIsHQCompaniesDropdownOpen(false); }}
                  className="p-2.5 rounded-xl hover:bg-[#f5eee4] cursor-pointer transition-all group flex items-start gap-3"
                >
                  <div className="w-8 h-8 rounded-lg bg-[#efe2d3] text-[#d96b27] group-hover:bg-[#d96b27] group-hover:text-white flex items-center justify-center shrink-0 transition-colors mt-0.5">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-stone-600 font-sans mt-0.5 leading-snug">
                      Premium vehicle rentals for comfortable Bhutan travel
                    </div>
                  </div>
                </div>

                <div 
                  onClick={() => { navigate('thangka-painting'); setIsHQCompaniesDropdownOpen(false); }}
                  className="p-2.5 pt-3 rounded-xl hover:bg-[#f5eee4] cursor-pointer transition-all group flex items-start gap-3"
                >
                  <div className="w-8 h-8 rounded-lg bg-[#efe2d3] text-[#d96b27] group-hover:bg-[#d96b27] group-hover:text-white flex items-center justify-center shrink-0 transition-colors mt-0.5">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-stone-600 font-sans mt-0.5 leading-snug">
                      Authentic traditional Bhutanese thangka art & paintings
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:hidden absolute top-full left-0 right-0 bg-[#fcf8f2] border-b-4 border-[#d96b27] shadow-2xl z-50 max-h-[80vh] sm:max-h-[85vh] overflow-y-auto">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6">
              ...
              <div className="space-y-2.5 font-serif text-sm text-[#3b2314]">
                ...
                {/* HQ COMPANIES ACCORDION */}
                <div className="rounded-xl border border-[#e2d1be] bg-[#f5eee4] overflow-hidden">
                  <button
                    onClick={() => setMobileHQCompaniesOpen(!mobileHQCompaniesOpen)}
                    className="w-full text-left py-3 px-3.5 flex items-center justify-between font-bold text-[#3b2314] hover:text-[#d96b27] cursor-pointer transition-colors"
                  >
                    <div className="flex items-center gap-2">
                      <Building2 className="w-4 h-4 text-[#d96b27]" />
                      <span>HQ Companies</span>
                    </div>
                    <ChevronDown className={`w-4 h-4 text-[#d96b27] transition-transform duration-200 ${mobileHQCompaniesOpen ? 'rotate-180' : ''}`} />
                  </button>

                  {mobileHQCompaniesOpen && (
                    <div className="px-3 pb-3 space-y-1.5 pt-1 border-t border-[#efe2d3]">
                      <button
                        onClick={() => { navigate('car-rental'); setMobileMenuOpen(false); }}
                        className="w-full text-left py-2.5 px-3 rounded-lg bg-white/80 hover:bg-white text-xs font-serif font-bold text-[#3b2314] hover:text-[#d96b27] flex items-center justify-between cursor-pointer"
                      >
                        <div className="flex items-center gap-2">
                          <Car className="w-3.5 h-3.5 text-[#d96b27]" />
                          <span>High Quality Car Rental</span>
                        </div>
                        <ChevronRight className="w-3.5 h-3.5 text-[#d96b27]" />
                      </button>

                      <button
                        onClick={() => { navigate('thangka-painting'); setMobileMenuOpen(false); }}
                        className="w-full text-left py-2.5 px-3 rounded-lg bg-white/80 hover:bg-white text-xs font-serif font-bold text-[#3b2314] hover:text-[#d96b27] flex items-center justify-between cursor-pointer"
                      >
                        <div className="flex items-center gap-2">
                          <Brush className="w-3.5 h-3.5 text-[#d96b27]" />
                          <span>High Quality Thangka Painting</span>
                        </div>
                        <ChevronRight className="w-3.5 h-3.5 text-[#d96b27]" />
                      </button>
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-3.5 px-4 text-center text-xs tracking-[0.15em] uppercase bg-[#25D366] text-white font-bold hover:bg-[#128C7E] transition-all cursor-pointer min-h-[44px] shadow-md rounded-lg flex items-center justify-center gap-2 sm:col-span-2"
                >
