...
import { 
  BarChart3, FileText, MessageSquare, Download, Package, Hotel, 
  Plus, Eye, ArrowUpRight, Clock, CheckCircle2, AlertCircle 
} from 'lucide-react';

export const AdminDashboardView: React.FC = () => {
  const { 
    packages, brochures, inquiries, hotels, 
      ...
      <div className="space-y-8">
        ...
        <div className="bg-amber-950 text-amber-50 rounded-2xl p-6 border border-amber-800 flex flex-wrap items-center justify-between gap-4 transition-all duration-300 hover:border-amber-600 hover:shadow-lg">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              onClick={() => navigate('admin-packages')}
              className="px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-amber-950 font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-sm transition-transform duration-200 hover:scale-[1.02]"
            >
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-amber-900 hover:bg-amber-850 border border-amber-700 text-amber-100 font-bold text-xs flex items-center gap-1.5 cursor-pointer transition-transform duration-200 hover:scale-[1.02]"
            >
