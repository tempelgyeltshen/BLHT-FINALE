...
import { 
  ...
  ChevronRight, Calendar, PhoneCall, ShieldCheck, Clock, Award, Shield, Check
} from 'lucide-react';
...
export const HotelDetailPage: React.FC = () => {
  ...
  const { 
    activeHotel, setActiveHotel, hotels, packages, setActivePackage,
    navigate
  } = useApp();

  // Fetch specific hotel details based on URL slug or id param, or fallback to activeHotel
  const urlSlug = slug || id || hotelId;
  const currentHotel = useMemo(() => {
    if (urlSlug) {
      const found = hotels.find(h => 
        h.id === urlSlug || 
        h.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') === urlSlug.toLowerCase()
      );
      if (found) return found;
    }
    return activeHotel || hotels[0];
  }, [urlSlug, activeHotel, hotels]);

  // Sync activeHotel state when found from URL
  useEffect(() => {
    if (currentHotel && currentHotel.id !== activeHotel?.id) {
      setActiveHotel(currentHotel);
    }
  }, [currentHotel, activeHotel?.id, setActiveHotel]);
